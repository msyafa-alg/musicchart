...
<head>
    ...
    <title>Albums - MusicChart Admin</title>
        ...
        <div class="flex justify-between items-center mb-8">
            <div>
                <h1 class="text-4xl font-black text-white mb-2">
                    <span class="gradient-text">Albums</span> Management
                </h1>
                <p class="text-gray-400">Manage your music album collection and metadata</p>
            </div>

            <div class="flex space-x-3">
                <!-- Export Button -->
                <a href="{{ route('albums.export') }}"
                ...
                <table class="w-full">
                    <thead class="table-header">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">Album Title</th>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">Artist</th>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">Popularity</th>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">Release Date</th>
...
<td class="px-6 py-4 whitespace-nowrap">
    ...
    @else
        ...
        <!-- DEBUG: Cover path: {{ $album->cover ?? 'No cover' }} -->
    @endif
</td>

                            <!-- Title -->
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-semibold text-white">{{ $album->judul }}</div>
                            </td>

                            <!-- Artist -->
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-300">{{ $album->artist->name ?? 'Unknown Artist' }}</div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center space-x-3">
                                    <div class="flex items-center space-x-2">
                                        ...
                                        <span class="text-sm font-bold text-white">{{ $album->popularity }}</span>
                                    </div>
                                    <div class="popularity-bar">
                                        <div class="popularity-fill" style="width: {{ $album->popularity }}%"></div>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td colspan="6" class="px-6 py-8 text-center">
                                <div class="flex flex-col items-center space-y-4">
                                    <div class="w-16 h-16 bg-gray-800 rounded-2xl flex items-center justify-center">
                                        <i class="fas fa-compact-disc text-gray-600 text-2xl"></i>
                                    </div>
                                    <div>
                                        <h3 class="text-lg font-semibold text-gray-400 mb-2">No albums found</h3>
                                        <p class="text-gray-500 text-sm">Get started by adding your first album</p>
                                    </div>
                                    <a href="{{ route('albums.create') }}"
                                       class="btn-primary px-6 py-2 text-sm">
                                        <i class="fas fa-plus mr-2"></i>Add First Album
                                    </a>
                                </div>
                            </td>
                        </tr>
