...
<head>
    ...
    <title>Songs Trash - MusicChart Admin</title>
    ...
    <div class="max-w-7xl mx-auto px-6 py-8">
        ...
        <div class="flex justify-between items-center mb-8">
            <div>
                <h1 class="text-4xl font-black text-white mb-2">
                    <span class="gradient-text">Songs Trash</span>
                </h1>
                <p class="text-gray-400">Manage deleted songs and restore if needed</p>
            </div>
                ...
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-gray-800">
                        ...
                        <tr class="table-row">
                            ...
                            <!-- Deleted At -->
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-400">{{ optional($song->deleted_at)->format('d M Y, H:i') ?? '-' }}</div>
                            </td>

                            <!-- Actions -->
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center space-x-3">
                                    <form action="{{ url('songs/' . $song->id . '/restore') }}" method="POST" class="inline">
                                        @csrf
                                        <button type="submit"
                                                class="action-btn restore-btn text-green-400 hover:text-green-300 flex items-center space-x-1">
                                            <i class="fas fa-undo"></i>
                                            <span class="text-sm">Restore</span>
                                        </button>
                                    </form>
                                    <form action="{{ url('songs/' . $song->id . '/force-delete') }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                                class="action-btn delete-btn text-red-400 hover:text-red-300 flex items-center space-x-1"
                                                onclick="return confirm('Permanently delete this song? This action cannot be undone!')">
        ...
        <div class="glass-card rounded-2xl p-12 text-center">
            <div class="flex flex-col items-center space-y-6">
                <div class="w-24 h-24 bg-gradient-to-br from-gray-700 to-gray-800 rounded-3xl flex items-center justify-center">
                    <i class="fas fa-trash-alt text-gray-500 text-4xl"></i>
                </div>
                <div>
                    <h2 class="text-2xl font-bold text-gray-400 mb-3">Trash is Empty</h2>
                    <p class="text-gray-500 text-lg">No deleted songs found in the trash</p>
                </div>
                <a href="{{ route('songs.index') }}"
                   class="btn-primary px-8 py-3 text-lg mt-4">
